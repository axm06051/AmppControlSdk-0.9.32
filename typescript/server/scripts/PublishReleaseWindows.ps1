...
$destinationPath  = ".\out\Packages\Windows\$($packageName)\$($version)"
 ...
 # Check if the target folder exists and delete it if it does
if (Test-Path -Path "./out/Packages" -PathType Container) {
    Write-Host "Deleting Ouput folder"
    Remove-Item -Path "./out/Packages" -Recurse -Force
}
...
compress-Archive -Path ".\out\Packages" -DestinationPath $zipName -Force
